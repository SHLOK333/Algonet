...
const fetchTokenMetadata = async (tokenType: string) => {
  if (tokenType === "pay_per_use") {
    return "https://developer.algorand.org/static/favicon-32x32.png";
  }
  return "https://www.artnews.com/wp-content/uploads/2022/01/unnamed-2.png";
};
...
const AuthRuleShowComponent = () => {
      ...
      try {
        ...
        const enrichedRules = await Promise.all(
          data.rules.map(async (rule: any) => {
            const imageUrl = await fetchTokenMetadata(rule.tokenType);
  ...
  return (
    <VStack spacing={6} align="center">
     <Text>Authentication supports pay-per-use x402 and optional ASA/NFT checks</Text>
